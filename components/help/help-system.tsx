    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="fixed bottom-4 right-4 h-10 w-10 rounded-full shadow-lg bg-white hover:bg-gray-100 text-black border-gray-200 z-50 transition-transform hover:scale-105"
        >
                ...
                {searchQuery ? (
                  ...
                  <div className="space-y-6">
                    {groupedContent && Object.entries(groupedContent).map(([category, articles]) => (
                      <div key={category}>
                        <h3 className="text-sm font-bold text-gray-900 mb-3 uppercase tracking-wider bg-gray-50 p-2 rounded-md inline-block">
                          {category}
                        </h3>
